...
-- Atualizar função get_admin_dashboard_data para contar TODAS as revendas da department_store
...
BEGIN
  ...
  -- Distributors: Count participants from participants table, revendas from department_store
  WITH 
  -- Participantes por distribuidor (fonte: participants)
  participantes_por_distribuidor AS (
    SELECT 
  ...
  -- Revendas por distribuidor (fonte: department_store + wallet para descobrir distribuidor)
  revendas_por_distribuidor AS (
    SELECT 
      COALESCE(w.distribuidor, 'Sem Registro') as representante,
      COUNT(DISTINCT ds.cliente_id) as total_revendas,
      COUNT(DISTINCT CASE 
        WHEN COALESCE(cv.vendas, 0) > 500 THEN ds.cliente_id 
      END) as revendas_ativas,
      COUNT(DISTINCT CASE 
        WHEN COALESCE(cv.vendas, 0) <= 500 THEN ds.cliente_id 
      END) as revendas_inativas,
  ...
  -- Juntar participantes e revendas usando FULL OUTER JOIN
  SELECT jsonb_agg(row_to_json(d)::jsonb ORDER BY d.vendas_totais DESC) INTO distributors_data
  FROM (
    SELECT 
      COALESCE(rpd.representante, ppd.representante) as representante,
      COALESCE(ppd.total_participantes, 0) as total_participantes,
      COALESCE(rpd.total_revendas, 0) as total_revendas,
      COALESCE(rpd.revendas_ativas, 0) as revendas_ativas,
      COALESCE(rpd.revendas_inativas, 0) as revendas_inativas,
      COALESCE(rpd.vendas_totais, 0) as vendas_totais
    FROM revendas_por_distribuidor rpd
    FULL OUTER JOIN participantes_por_distribuidor ppd ON rpd.representante = ppd.representante
  ) d;
  ...
  -- Buscar consultores na wallet que não são participantes
